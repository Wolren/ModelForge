"""
ForgeGenerateWorker - background thread for model generation.
"""
...
class ForgeGenerateWorker(QThread):
    """Worker thread for generating models in background."""
    ...
    def run(self):
        ...
        from ..compiler_core.core.context_collector import CompilerContextCollector
        from ..compiler_core.core.llm.factory import create_compiler_backend
        from ..compiler_core.core.mcp.server import build_server
        from ..compiler_core.core.mcp.client import DirectMCPClient
        from .services.layout_service import GraphLayoutService
    ...
    def _build_attempt_context(self, full_context, attempt):
        ...
        limit = min(len(algo_items), 40 if attempt == 1 else 20)
    ...
    def _build_attempt_prompt(self, base_text, last_error, attempt):
        if attempt <= 0 or not last_error:
            return base_text
        return f"{base_text}\n\nRetry notes: {last_error}. Use only algorithms present in the catalog. Return a compact, valid plan."

    def _friendly_error_text(self, error):
        from ..compiler_core.core.llm.base import LLMBackendError, LLMTimeoutError
        if isinstance(error, LLMTimeoutError):
            return "The LLM request timed out after optimized retries. Try a smaller model context or increase backend timeout."
