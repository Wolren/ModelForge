...
class LayoutConfig:
    ...
    input_x:   float  = 80.0    # x position of model inputs column
    start_x:   float  = 380.0   # x origin of first algorithm rank

    @classmethod
    def compact(cls)  -> "LayoutConfig":
        return cls(h_spacing=280.0, v_spacing=100.0, input_x=60.0, start_x=340.0)

    @classmethod
    def balanced(cls) -> "LayoutConfig":
        return cls(h_spacing=300.0, v_spacing=120.0, input_x=80.0, start_x=380.0)

    @classmethod
    def dense(cls)    -> "LayoutConfig":
        return cls(h_spacing=250.0, v_spacing=80.0, input_x=50.0, start_x=300.0)

    @classmethod
    def spacious(cls) -> "LayoutConfig":
        return cls(h_spacing=520.0, v_spacing=180.0, input_x=100.0, start_x=440.0)

    @classmethod
    def debug(cls)    -> "LayoutConfig":
        return cls(h_spacing=420.0, v_spacing=160.0, input_x=90.0, start_x=400.0)
...
class GraphLayoutService:
    ...
    @staticmethod
    def _estimate_wrap_lines(text: str, wrap_width: int = 28) -> int:
        ...
        for line in logical_lines:
            ...
            lines += max(1, (line_len + wrap_width - 1) // wrap_width)
        return max(1, lines)
