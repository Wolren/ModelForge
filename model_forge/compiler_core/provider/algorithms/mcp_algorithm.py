...
if _HAS_QGIS:
    ...
    class ModelForgeMCPAlgorithm(QgsProcessingAlgorithm):
        ...
        def name(self) -> str:
            return "mcp_build_workflow"

        def displayName(self) -> str:
            return self.tr("Build Workflow from Description (MCP)")
        ...
        def createInstance(self):
            return ModelForgeMCPAlgorithm()

        def initAlgorithm(self, config=None):
            self.addParameter(
                QgsProcessingParameterString(
                    self.INPUT_DESCRIPTION,
                    self.tr("Workflow description"),
                    multiLine=True,
                )
            )
            self.addParameter(
            ...
            self.addOutput(
                QgsProcessingOutputString(self.OUTPUT_JSON, self.tr("Generated model JSON"))
            )
        ...
        def processAlgorithm(self, parameters, context, feedback):
            ...
            feedback.pushInfo(
                f"Starting ModelForge MCP pipeline (provider={provider}, model={llm_model})"
            )
            ...
            from ...core.services.registry.registry_catalog import (
            ...
            pipeline = CompilerPipeline(
                ...
                link_repair=LinkRepairService(registry_catalog=catalog_svc),
            )
            ...
            feedback.pushInfo(
                f"Compiler pipeline complete. Steps: {len(plan.steps)}, Issues: {len(plan.issues)}"
            )
