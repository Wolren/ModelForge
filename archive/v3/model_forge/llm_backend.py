...
3. Each plan step MUST have:
   ...
   - "inputs_from": list containing either:
       - "model_input" to indicate direct use of model-level inputs
       - or step indices (integers) whose outputs feed this step
...
5. If the task can be done with native:extractbyexpression instead of multiple
...
SYSTEM_PROMPT_REPAIR = """You are a QGIS Processing Model debugger. Given a model JSON and validation errors, fix the JSON.

CRITICAL RULES:
1. Output ONLY the corrected full JSON. No markdown, no code fences, no commentary.
2. Fix ONLY what the errors describe. Do not restructure or add steps.
3. Common fixes:
...
4. If user feedback is included, address that too.
...
class LLMBackend:
    ...
    def _parse_json_response(self, text):
        ...
        3. Extract first top‑level {...} block and parse.
        4. Apply a couple of safe repairs (trailing commas).
        5. As a last resort, progressively trim and close braces/brackets.
        If everything fails, raise ValueError but include a cleaned snippet
        so the user can inspect what the LLM actually returned.
        """
        original_text = text
        text = text.strip()
        ...
        # 4) Safe repair: remove trailing commas before } or ]
        ...
        # 5) Last resort: progressive trimming + auto‑closing
        ...
        # 6) Give up, but include a readable snippet to help debugging
